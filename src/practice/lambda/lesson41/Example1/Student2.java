package practice.lambda.lesson41.Example1;
//Пример, который можно было бы упростить, используя интерфейс и лямбда-выражения
...
class Student2Info {
//    //Для использования лямбда-выражений всю однотипную часть методов мы
//    // прописываем здесь: в главном методе, в параметры которого вставляем
//    // - множество объектов, над которыми совершаем действие - и объект
//    // класса, который имплементирует наш функциональный интерфейс. И теперь
//    //каждый следующий раз, когда нам понадобится применить очередной новый
//    // метод, мы будем использовать данный метод, но в место объекта класса,
//    // имплементирующего интерфейс, будем прописывать анонимный класс либо
//    // его сокращенную версию: лямбда-выражение (см. ниже пункт 2), 3)).
...
//        sInfo.testStudents(students, new CheckByName());//используем класс, который имплементирует интерфейс StudentChecks
//        // (способ 1), специально созданный для нужного нам сейчас метода
//        System.out.println("-------------------");
//        sInfo.testStudents(students, new StudentChecks() { //...2) используем анонимный класс
//            // (имплементирующий интерфейс StudentChecks)
//            @Override
//            public boolean check(Student2 s) {
//                return s.name.equals("Ivan");
//            }
